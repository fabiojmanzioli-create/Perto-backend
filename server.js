...
app.post('/webhook/hotmart', async (req, res) => {
  try {
    ...
    if (!isAprovado) return res.status(200).json({ ok: true, msg: 'evento ignorado' });
    ...
    db.prepare(`INSERT INTO tokens (token, email, nome, criado_em, expira_em) VALUES (?, ?, ?, ?, ?)`)
...
app.post('/admin/gerar-token', async (req, res) => {
  try {
    ...
    if (adminKey !== process.env.ADMIN_KEY) return res.status(401).json({ error: 'não autorizado' });
...
app.get('/acesso', (req, res) => {
  ...
  if (!token) return res.send(paginaErro('Link inválido', 'Nenhum token foi fornecido. Verifique o link enviado por e-mail.'));

  const row = db.prepare('SELECT * FROM tokens WHERE token = ?').get(token);
  if (!row) return res.send(paginaErro('Link não encontrado', 'Este link não existe. Verifique o e-mail que recebeu ou entre em contato com o suporte.'));
...
app.post('/salvar-resultado', (req, res) => {
  ...
  db.prepare(`INSERT OR REPLACE INTO respostas (token, perfil, extroversao, estabilidade, dados_json) VALUES (?, ?, ?, ?, ?)`)
